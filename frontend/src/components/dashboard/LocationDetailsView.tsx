...
import { Camera, Check, Heart, History, MoreVertical, Star, UserCircle2 } from "lucide-react";
...
const LocationDetailsView: React.FC<LocationDetailsViewProps> = ({
  ...
  return (
    <div onClick={() => { setIsDropdownOpen(false); setIsWishlistDropdownOpen(false); }}>
      <div className="mb-6">
        <button
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleLocationCollection(currentLocation.id, "travelHistory");
                          setIsDropdownOpen(false);
                        }}
                        className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition"
                      >
        ...
        {hasValidCoordinates ? (
          ...
          <p className="mt-4 text-sm text-amber-600 dark:text-amber-400">
            Map unavailable: this location is missing valid latitude/longitude.
          </p>
        )}
        ...
        {locationReviews && locationReviews.length > 0 ? (
          <div className="flex flex-col gap-4">
            {locationReviews.map((review, idx) => {
              ...
              return (
                <div key={idx} className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                  <div className="flex items-start gap-4 mb-3">
                    ...
                    <div className="flex-1">
                      <p className="font-semibold text-slate-800 dark:text-slate-100">{review.user_name}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{review.travel_date}</p>
                    </div>
                    {review.rating && (
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            size={14}
                            className={star <= (review.rating ?? 0) ? "text-yellow-400 fill-yellow-400" : "text-slate-300 dark:text-slate-600"}
                          />
                        ))}
                      </div>
                    )}
          ...
          <p className="text-slate-500 dark:text-slate-400 italic">No public reviews yet for this location.</p>
        )}
      </div>
    </div>
  );
